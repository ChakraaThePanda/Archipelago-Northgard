...
fn write_variant(enum_name: &Ident, v: &Variant, i: u8) -> TokenStream {
    ...
    let fwrite = v.fields.iter().map(|f| {
        ...
        match ident(&f.ty).as_str() {
            ...
            // BUGFIX: these two were previously unhandled (fell through to the `_`
            // wildcard, which writes nothing), silently dropping the field's bytes on
            // write while the read side still expects them -- corrupting every opcode
            // that carries an InlineBool/InlineInt (e.g. `Bool`), and desyncing every
            // byte after it in the file. Mirrors read_variant's read_vari-based decoding.
            "InlineInt" => quote! {
                write_var(w, *#fname)?;
            },
            "InlineBool" => quote! {
