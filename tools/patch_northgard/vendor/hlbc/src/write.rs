...
pub(crate) fn write_strings(w: &mut impl Write, strings: &[Str]) -> Result<()> {
    // NOTE: deliberately not using CString here -- real HL strings are length-prefixed
    // and may contain embedded null bytes (confirmed: at least one string in Northgard's
    // own bytecode does), which CString::new() rejects with a NulError. Write raw bytes +
    // a manual null terminator instead, matching the actual on-disk format.
    let size = strings
        .iter()
